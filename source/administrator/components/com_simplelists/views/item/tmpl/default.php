			...
			<ul class="nav nav-tabs">
				<li>
					<a href="#basic" data-toggle="tab">
						<?php echo JText::_('COM_SIMPLELISTS_ITEM_FIELDSET_BASIC'); ?>
					</a>
				</li>
				<li>
					<a href="#text" data-toggle="tab">
						<?php echo JText::_('COM_SIMPLELISTS_ITEM_FIELDSET_TEXT'); ?>
					</a>
				</li>
				<li><a href="#image" data-toggle="tab">
						<?php echo JText::_('COM_SIMPLELISTS_ITEM_FIELDSET_IMAGE'); ?>
					</a>
				</li>
				<li>
					<a href="#link" data-toggle="tab">
						<?php echo JText::_('COM_SIMPLELISTS_ITEM_FIELDSET_LINK'); ?>
					</a>
				</li>
				<li><a href="#params" data-toggle="tab">
						<?php echo JText::_('COM_SIMPLELISTS_ITEM_FIELDSET_PARAMS'); ?>
					</a>
				</li>
			</ul>
							...
							<div class="control-group">
								<div class="controls">
									<input type="text" id="picture_name" name="picture_name"
									       value="<?php echo $this->item->picture; ?>">
									<input type="hidden" id="picture" name="picture"
									       value="<?php echo $this->item->picture; ?>"/>
								</div>
								<div class="blank">
									<a class="btn modal-button"
									   title="<?php echo JText::_('COM_SIMPLELISTS_SELECT_IMAGE') ?>"
									   href="<?php echo $this->modal['picture'];
								...
								<div class="image-preview">
									<?php if (!empty($this->item->picture_path) && JFile::exists($this->item->picture_path)): ?>
										<img width="380" id="picture-preview"
										     src="../<?php echo $this->item->picture_uri; ?>" name="item_picture"/>
									<?php else: ?>
										<img width="380" id="picture-preview"
										     src="../media/com_simplelists/images/blank.png"
										     alt="<?php echo JText::_('No image'); ?>" name="item_picture" width="1"
										     height="1"/>
									<?php endif; ?>
								</div>
							</div>
						...
						<?php if (!empty($this->link_plugins)) : ?>
							<div class="control-group">
								<?php foreach ($this->link_plugins as $plugin) : ?>
									<?php /** @var $plugin SimplelistsPluginLink */ ?>
									<?php if ($plugin->isEnabled() == false) :
										continue;
									endif; ?>
									<?php $link_type = $plugin->getPluginName(); ?>
									<?php $active = ($link_type == $this->item->link_type) ? true : false; ?>
									<label class="simplelists_link radio" for="link_type_<?php echo $link_type; ?>"
									       id="link_type_<?php echo $link_type; ?>_label">
										<?php $checked = ($active) ? 'checked="checked"' : ''; ?>
										<input type="radio" class="simplelists_link"
										       id="link_type_<?php echo $link_type; ?>" name="link_type"
										       value="<?php echo $link_type; ?>" <?php echo $checked; ?> />
										<?php echo $plugin->getTitle(); ?>
									</label>
									<?php $classes = array(
										'simplelists_link_inner',
										'simplelists_link_inner_type_' . $link_type
									); ?>
									<?php if ($active)
									{
										$classes[] = 'simplelists_link_inner_active';
									} ?>
									<div class="<?php echo implode(' ', $classes); ?>"
									     id="simplelists_link_inner<?php echo $link_type; ?>">
