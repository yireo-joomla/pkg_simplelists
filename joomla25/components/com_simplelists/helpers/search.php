...
class SimplelistsHelperSearch
{
    ...
    public function search($text, $phrase = '', $ordering = '', $limit)
    {
        ...
        $query = 'SELECT s.id, s.title, s.alias, s.text AS text, b.category_id, c.alias AS category_alias, c.title AS catname,'
        . ' "2" AS browsernav'
        . ' FROM #__simplelists_items AS s'
        . ' LEFT JOIN #__simplelists_categories AS b ON b.id = s.id'
        . ' LEFT JOIN #__categories AS c ON c.id = b.category_id'
        . ' WHERE ('.$where.')'
        . ' AND c.published = 1'
        . ' AND s.published = 1'
        . ' GROUP BY s.id'
